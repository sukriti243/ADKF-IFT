...
from fs_mol.models.adaptive_dkt_prior_1 import ADKTPriorModel, ADKTPriorModelConfig
...
def evaluate_adkt_model(
    ...
    def test_model_fn(
        ...
        for batch_features, this_batch_labels, this_batch_numeric_labels in zip(dkt_task_sample.batches, dkt_task_sample.batch_labels, dkt_task_sample.batch_numeric_labels):
...
class ADKTPriorModelTrainer(ADKTPriorModel):
    def __init__(self, config: ADKTPriorModelTrainerConfig):
        ...
        self.inner_optimizer = torch.optim.Adam([{'params': self.feature_extractor_params(), 'lr': config.learning_rate}, 
                                                 {'params': self.gp_params(), 'lr': config.learning_rate}])
        self.outer_optimizer = torch.optim.Adam([{'params': self.prior_params()}], config.learning_rate*0.1)
