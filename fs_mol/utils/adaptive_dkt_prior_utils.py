...
class ADKTPriorModelTrainer(ADKTPriorModel):
    ...
    def train_loop(self, out_dir: str, dataset: FSMolDataset, device: torch.device, aml_run=None):
        ...
        for step in range(1, self.config.num_train_steps + 1):
            ...
            for _ in range(self.config.tasks_per_batch):
                ...
                for batch_features, this_batch_labels, this_batch_numeric_labels in zip(batches, batch_labels, batch_numeric_labels):
                    ...
                    with higher.innerloop_ctx(self, self.inner_optimizer, track_higher_grads=False) as (fmodel, diffopt):
                        ...
                        for step in range(fmodel.config.num_inner_iters):
                        ...
                        params = list(islice(fmodel.parameters(time=-1), 379, 383 ,1))
                        hparams = list(islice(fmodel.parameters(), 0, 8, 1))
            ...
            if (self.config.num_train_steps%1==0):
                print("Epoch {}: Loss {}".format(self.config.num_train_steps, task_batch_mean_loss))
