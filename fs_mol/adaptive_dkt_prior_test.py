...
from fs_mol.models.adaptive_dkt_prior_1 import ADKTPriorModel
...
def main():
    ...
    out_dir, dataset = set_up_test_run("ADKTPriorModel", args, torch=True)
    ...
    model_weights_file = resolve_starting_model_file(
        ...
        model_cls=ADKTPriorModel,
        out_dir=out_dir,
        use_fresh_param_init=args.use_fresh_param_init,
        device=device,
    )

    model = ADKTPriorModelTrainer.build_from_model_file(
        model_weights_file,
        device=device,
    ).to(device)
