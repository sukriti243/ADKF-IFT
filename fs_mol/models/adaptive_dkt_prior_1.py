...
class ADKTPriorModel(nn.Module):
    ...
    def feature_extractor_params(self):
        fe_params = []
        for name, param in self.named_parameters():
            if not name.endswith("_nn") and not name.startswith("gp_") and not "mp_norm_layer" in name:
    ...
    def __create_tail_GP(self, kernel_type):
        ...
        if self.config.use_numeric_labels:
            ...
            noise_prior = gpytorch.priors.LogNormalPrior(loc=loc, scale=scale, validate_args = False)
        else:
            scale = 0.25
