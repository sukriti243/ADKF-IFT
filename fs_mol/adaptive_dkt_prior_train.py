...
from fs_mol.utils.adaptive_dkt_prior_utils_1 import (
    ADKTPriorModelTrainerConfig,
...
def parse_command_line():
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="Number of training steps in inner loop"
...
def make_trainer_config(args: argparse.Namespace) -> ADKTPriorModelTrainerConfig:
    return ADKTPriorModelTrainerConfig(
...
def main():
    ...
    out_dir, dataset, aml_run = set_up_train_run(
        f"ADKTPriorModel_{config.used_features}", args, torch=True
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_trainer = ADKTPriorModelTrainer(config=config).to(device)
